...
这样即可 "file.in" 里的所有代码**直接**包含进入当前文件之中。在本仓库的 `lib` 文件夹里提供了一些库文件。例如，使用 `map` 库文件：
...
如果不 include 对应的库文件，将会因为找不到 `Map` 类而报告错误。

请注意，`include` **不属于语句**，属于编译期指令。另外，**RBQScript 不会检查是否出现了循环 include**。
