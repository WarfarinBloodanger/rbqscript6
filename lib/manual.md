...
有时候需要遍历一个列表或者一个对象的全部值 / 全部成员。对于列表，虽然可以用 `len` 函数获取到它的长度再进行遍历，但是需要更多的代码。

RBQScript 提供了 for-all 语句来简洁地完成这项工作。格式如下：

```
for all(i:iterable){}
```

其中，`iterable` 是一个支持遍历的对象。支持遍历的对象包含：string、数组、对象。这段代码将会让 `i` 遍历 `iterable` 中的**每一个值**。如下代码：

```
for all(i:Math) Console.print(i)
```

运行上面这段代码，将会把 `Math` 这个对象中的所有成员都输出出来。输出的前几行如下：
...
有些时候需要使用多行文本，使用多个 `""` 字符串连接起来不是很方便。此时可以用多行字符串来一次性完成工作：
...
当需要使用大量反斜杠 `\` 的时候，大量写上 `"\\"` 不是一个很好的主意。RBQScript 提供无转义字符串，格式为：

```
`abc\def\'`
```

使用反引号开头的字符串内部的 `\` 将不进行转义。

## 注释

RBQScript 提供单行注释功能。从两个双斜杠 `//` 到当行末尾的所有字符都是注释。请注意，注释会被编译器忽略，注释仅供辅助代码阅读之用。
...
这份代码将会输出 1。因为 `a=2` 在注释之中，被编译器忽略。

RBQScript 还提供多行注释功能。如果使用 `/*` 和 `*/` 将一段文字括起来，这段文字将全部被视作注释。如下：
...
这份代码将会输出 `1`。中间的两条语句均属于注释。

## 预处理指令

预处理指令都是使用 `#` 开头的行。请注意，预处理指令**不属于语句**，属于编译期指令。

### 文件包含

RBQScript 虽然提供了一部分内置函数，但是有时候仍然需要更多的功能；又或者您需要和其他开发者一同开发项目，需要共享代码。此时就可以使用 RBQScript 的文件包含功能：
...
这样即可 `file.rin` 和 `map.rin` 里的所有代码**直接**包含进入当前文件之中。在本仓库的 `lib` 文件夹里提供了一些库文件。例如，使用 `map` 库文件：
...
如果不 include 对应的库文件，将会因为找不到 `Map` 类而报告错误。另外，**RBQScript 不会检查是否出现了循环 include**。
